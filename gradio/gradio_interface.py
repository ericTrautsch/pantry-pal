...
def launch_interface(username, access_token, port):
    ...
    iface = Interface(
        ...
        inputs=[
            ...
            CheckboxGroup(
                label="Pantry Items", choices=fetch_pantry_items(username, access_token)
            ),
            TextArea(
                label="Macro Goals (optional)",
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--username", required=True, type=str, help="Username for authentication"
    )
    parser.add_argument(
        "--access_token",
        required=True,
