...
def initialize_model():
    ...
    sample = client.generate(
        model=ollama_model, prompt="Hello! Respond with only Hello."
    )
...
def get_meal_suggestions(user_id: str, daily_macro_goals: InventoryItemMacros):
    ...
    logging.info(
        f"Generating meal suggestions for user ID: {user_id} with daily macro goals: {daily_macro_goals}"
    )
    items = read_pantry_items(user_id)
...
def generate_recipe_prompt(items):
    ...
    for item in items:
        ...
        macros_str = ", ".join(
            f"{key}: {value}" for key, value in macros.items() if value
        )
