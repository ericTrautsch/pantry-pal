...
def test_get_recipe_recommendations(
    mock_read_pantry_items, mock_client_generate, auth_headers
):
    logging.info("Testing get_recipe_recommendations endpoint")
    mock_read_pantry_items.return_value = [
        {"product_name": "Tomato"},
        {"product_name": "Cheese"},
    ]
    ...
    response = requests.get(
        f"{API_BASE_URL}/ai/meal_recommendation?user_id=testuser", headers=auth_headers
    )
...
def test_get_meal_suggestions(
    ...
    response = requests.post(
        f"{API_BASE_URL}/ai/meal_suggestions?user_id=testuser",
