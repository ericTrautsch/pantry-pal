...
def import_pantry(file):
    ...
    try:
        ...
        for item in items:
            if not any(
                existing_item["product_name"] == item["product_name"]
                for existing_item in fetch_pantry_items(
                    user["id"], st.session_state["token"]
                )
            ):
                add_pantry_item(
                    user["id"],
...
if user is None:
    ...
    with st.sidebar:
        ...
        st.markdown(
            f"#### **{current_datetime.strftime('%A, %B %d, %Y')}**"
        )  # Bold date
        ...
        with st.form("add_item_form"):
            ...
            if submitted:
                ...
                if add_pantry_item(
                    user["id"], name, quantity, st.session_state["token"]
                ):
    ...
    def delete_item(item_id):
        if delete_pantry_item(user["id"], item_id, st.session_state["token"]):
            logging.info(
                f"Item ID: {item_id} deleted successfully by user {user['username']}"
            )
            ...
            logging.error(
                f"Failed to delete item ID: {item_id} for user {user['username']}"
            )
            ...
            for item in items:
                ...
                with st.expander(
                    ...
                    if not st.session_state.get(f"delete_confirm_{item_id}", False):
                        ...
                        st.warning(
                            f"Are you sure you want to delete **{item['product_name']}**?"
                        )
